...
-- Create tables to demonstrate the differences
...
SELECT name, attributes FROM products WHERE name = 'Smartphone X';
...
-- Check the updated value
SELECT name, profile->'preferences'->>'theme' FROM customers WHERE name = 'John Smith';
...
-- Check the updated value
SELECT name, jsonb_array_length(profile->'payment_methods') FROM customers WHERE name = 'John Smith';
...
SELECT 
    name, 
    jsonb_array_elements(profile->'payment_methods')->>'type' AS payment_type 
FROM 
    customers 
WHERE 
    name = 'John Smith';
...
SELECT 
    name, 
    jsonb_array_length(orders) AS orders_count 
FROM 
    customers 
WHERE 
    name = 'Jane Doe';
...
EXPLAIN ANALYZE SELECT * FROM products WHERE attributes->>'storage' = '128GB';

-- Query that can use idx_product_attributes
EXPLAIN ANALYZE SELECT * FROM products WHERE attributes @> '{"color": "red", "storage": "128GB"}';

-- Query that can use idx_product_specs
EXPLAIN ANALYZE SELECT * FROM products WHERE specifications @> '{"processor": "Intel Core i7"}';

-- Query that can use idx_customer_profile
EXPLAIN ANALYZE SELECT * FROM customers WHERE profile @> '{"address": {"city": "New York"}}';

-- Query that can use idx_customer_orders
EXPLAIN ANALYZE SELECT * FROM customers WHERE orders @> '[{"status": "delivered"}]';
...
-- Example 9: JSON Functions Overview
-- Create a test table
CREATE TABLE json_functions_demo (
    id SERIAL PRIMARY KEY,
    data JSONB
);

INSERT INTO json_functions_demo (data) VALUES
('{"name": "Alice", "age": 30, "tags": ["developer", "postgresql"], "address": {"city": "New York", "zip": "10001"}}');

-- Check if a key exists
SELECT 
    data ? 'name' AS has_name,
    data ? 'salary' AS has_salary
FROM 
    json_functions_demo;

-- Check if any of multiple keys exist
SELECT 
    data ?| ARRAY['name', 'salary'] AS has_any_key
FROM 
    json_functions_demo;

-- Check if all keys exist
SELECT 
    data ?& ARRAY['name', 'age'] AS has_all_keys
FROM 
    json_functions_demo;
...
FROM 
    json_functions_demo,
    jsonb_to_record(data) AS x(name text, age int, address jsonb);
...
-- Build JSON objects
SELECT 
    jsonb_build_object('firstName', 'John', 'lastName', 'Doe', 'age', 42);

-- Build JSON arrays
SELECT 
    jsonb_build_array('apple', 'banana', 'orange');

-- Merge JSON objects
SELECT 
    data || '{"salary": 75000, "department": "IT"}'::jsonb AS merged_data
FROM 
    json_functions_demo;

-- Check containment (useful with GIN indexes)
SELECT 
    data @> '{"name": "Alice"}'::jsonb AS contains_name,
    data @> '{"address": {"city": "New York"}}'::jsonb AS contains_address
FROM 
    json_functions_demo;

-- Check if contained within
SELECT 
    '{"name": "Alice"}'::jsonb <@ data AS is_contained
FROM 
    json_functions_demo;

-- Concatenate JSON arrays
SELECT 
    jsonb_build_array('a', 'b') || jsonb_build_array('c', 'd') AS concat_arrays;
...
SELECT 
    ...
    jsonb_typeof(data->'address') AS address_type
FROM 
    json_functions_demo;
...
DROP TABLE IF EXISTS jsonb_example;
DROP TABLE IF EXISTS products;
DROP TABLE IF EXISTS customers;
DROP TABLE IF EXISTS validated_documents;
DROP FUNCTION IF EXISTS validate_document_schema;
DROP FUNCTION IF EXISTS validate_json_schema;
DROP TABLE IF EXISTS event_logs;
DROP TABLE IF EXISTS form_submissions;
DROP TABLE IF EXISTS app_configurations;
DROP TABLE IF EXISTS json_functions_demo;
*/
