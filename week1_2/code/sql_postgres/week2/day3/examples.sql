...
EXPLAIN ANALYZE SELECT * FROM users WHERE active = true AND user_id < 1000;

-- This query cannot use the partial index
EXPLAIN ANALYZE SELECT * FROM users WHERE active = false AND user_id < 1000;

-- Create a partial index for recently active users
CREATE INDEX idx_recent_login ON users(last_login) 
WHERE last_login > current_date - INTERVAL '30 days';

-- This query can use the partial index
EXPLAIN ANALYZE SELECT * FROM users 
WHERE last_login > current_date - INTERVAL '15 days';

-- Example 7: Functional Indexes
-- Create an index on lowercase email for case-insensitive searches
CREATE INDEX idx_users_email_lower ON users(LOWER(email));

-- Query that can use the functional index
EXPLAIN ANALYZE SELECT * FROM users WHERE LOWER(email) = 'john.smith1@gmail.com';

-- Create an index on the year of birth
CREATE INDEX idx_users_birth_year ON users(EXTRACT(YEAR FROM date_of_birth));

-- Query that can use the functional index
EXPLAIN ANALYZE SELECT * FROM users WHERE EXTRACT(YEAR FROM date_of_birth) = 1980;

-- Clean up (comment out if you want to keep the tables for further exploration)
-- DROP TABLE users;
