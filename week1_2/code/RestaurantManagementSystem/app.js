...
// This file provides the console interface for the restaurant management system

const readline = require('readline');
const { menuService, orderService, tableService, reservationService } = require('./models');
...
function updateMenuItem() {
    rl.question('\nEnter the ID of the item to update: ', (itemIdStr) => {
        ...
        rl.question('Enter new name (or press Enter to keep current): ', (name) => {
            const newName = name.trim() === '' ? item.name : name;
            
            rl.question('Enter new price (or press Enter to keep current): ', (priceStr) => {
                ...
                if (priceStr.trim() !== '') {
                    ...
                    if (!isNaN(parsedPrice) && parsedPrice > 0) {
                        ...
                        console.log('Invalid price. Keeping the current price.');
                    }
                }
                
                rl.question('Enter new description (or press Enter to keep current): ', (description) => {
                    const newDescription = description.trim() === '' ? item.description : description;
                    ...
                    const success = menuService.updateMenuItem(itemId, updatedItem);
...
function deleteMenuItem() {
    rl.question('\nEnter the ID of the item to delete: ', (itemIdStr) => {
        const itemId = parseInt(itemIdStr);
        const item = menuService.getMenuItemById(itemId);
...
function updateReservation() {
    ...
    rl.question('\nEnter reservation ID to update: ', (reservationIdStr) => {
        ...
        rl.question('\nEnter new customer name (or press Enter to keep current): ', (customerName) => {
            const newName = customerName.trim() === '' ? reservation.customerName : customerName;
